    ...
    return (
        ...
        >
            {images.map((src: string, idx: number) => (
                <div
                    key={idx}
                    className={`card card-${idx} absolute w-[200px] aspect-square border-8 border-white dark:border-zinc-800 rounded-[30px] overflow-hidden`}
                    style={{
                        boxShadow: '0 4px 20px rgba(0, 0, 0, 0.2)',
                        transform: transformStyles[idx] || 'none',
                        zIndex: images.length - Math.abs(2 - idx) // middle card highest z-index
                    }}
